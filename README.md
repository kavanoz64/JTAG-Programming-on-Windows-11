...
nothing on the chain, which is indistinguishable from a wiring fault. Use the
...
work here, because the colon in the filespec (`design.jed:v`) stops MSYS2 translating
...
filename. For `cd` itself, MSYS2 paths work fine, because it's a shell builtin:
...
change. For a generic board, follow Part 4 exactly but pick the interface
...
`RST` sits immediately next to `AD0` on the header, so count carefully. TCK
...
xc3sprog's `-s <serial>` won't help, because there is no serial to match.
...
Not a signal problem. The old D2XX backend polls an empty read FIFO, so counts
go *up* as you lower the clock. The libftdi1 build reports `retries 0`. Only chase
...
(`-J 500000` gives 461.538 kHz, which is expected).
...
targets (e.g. Colorlight i5) using `--cable tigard` with the appropriate SRAM
or flash flags. No separate setup needed.
